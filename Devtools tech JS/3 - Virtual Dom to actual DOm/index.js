...
function renderToDOM(virtualNode) {
  if (!virtualNode) return; // Handle null or undefined nodes
  let fragement = document.createDocumentFragment()
  if (Array.isArray(virtualNode)) {
    let temp_fragement = document.createDocumentFragment()
    for (let node of virtualNode) {
      let child = renderToDOM(node); // Handle arrays of nodes
      temp_fragement.appendChild(child)
    }
    return temp_fragement;
  }
  ...
  if (props) {
    for (let key in props) {
      if (key === "children") {
        ...
        if (typeof children === "object") {
          for (let childKey in children) {
            let child = renderToDOM(children[childKey]);
      ...
      4:{
        ...
        props: {
          ...
          children: [
            {
              type: "span",
              props: { children: "This is a span inside a div." },
            },
            {
              type: "ul",
              props: {
...
let getChildNodes = renderToDOM(virtualNode);
domNode.appendChild(getChildNodes);
